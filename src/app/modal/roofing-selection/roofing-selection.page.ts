...
export class RoofingSelectionPage implements OnInit {
  ...
  updateDescription2() {
    let data = this.roofingObservationContent;
    this.descOutput = [];
    data.map((obj, index) => {
      obj.data.map((item) => {
        if (item.checked) {
    ...
    if (this.descOutput.length) {
      this.StoredData.roofingObservation = this.descOutput;
    }
    console.log(this.StoredData);
    this.config.storageRemoveItem('InspectionToEdit');
    this.config.storageSave('InspectionToEdit', this.StoredData);
  ...
  updateDescription5() {
    let data = this.roofingDescriptionContent;
    this.descOutput = [];
    data.map((obj, index) => {
      obj.data.map((item) => {
        if (item.checked) {
    ...
    console.log('desc output ', this.descOutput);
    if (this.descOutput.length) {
      this.StoredData.roofingDescriptionContent = this.descOutput;
    }
    console.log(this.StoredData);
    this.config.storageRemoveItem('InspectionToEdit');
    this.config.storageSave('InspectionToEdit', this.StoredData);
  ...
  updateStorage() {
    const data = this.roofingDescriptionContent;
    console.log('HERE IS SAVING NOW----->>>>>>');
    console.log(data);

    this.config.storageSave('roofingDescriptionContent', data);
  }

  updateStorage2() {
    const data = this.roofingObservationContent;
    // const data = JSON.stringify(this.structureObservation);
    this.config.storageSave('roofingObservationContent', data);
  }

  updateStorage3() {
    const data = this.RoofingLimitationsContent;
    // const data = JSON.stringify(this.structureObservation);
    this.config.storageSave('RoofingLimitationsContent', data);
  }
  ...
  addDescription(index, data) {
    const selectedItem = this.roofingDescriptionContent[index];
  ...
  addDescription2(index, data) {
    ...
    const selectedItem = this.roofingObservationContent[index];
  ...
  addDescription3(index, data) {
    ...
    const selectedItem = this.RoofingLimitationsContent[index];
    ...
    if (this.descOutput.length) {
      ...
      if (arrObj.length) {
        ...
        let obj = {
          content: [item],
          font_color:
            this.roofingObservationContent[index].font_color || '#000000',
          font_size: this.roofingObservationContent[index].font_size || 12,
          title: this.roofingObservationContent[index].title,
          id: index,
        };
    ...
    if (this.descOutput.length) {
      ...
      if (arrObj.length) {
        ...
        let obj = {
          content: [item],
          font_color:
            this.roofingDescriptionContent[index].font_color || '#000000',
          font_size: this.roofingDescriptionContent[index].font_size || 12,
          title: this.roofingDescriptionContent[index].title,
          id: index,
        };
        this.descOutput.push(obj);
      }
